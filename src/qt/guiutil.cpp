...
{
    // Convert egame:// to egame:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("egame://"))
    {
        uri.replace(0, 12, "egame:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "EGame.lnk";
}
...
{
    return GetAutostartDir() / "egame.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("EGame-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  egame-qt [" + tr("command-line options") + "]                     " + "\n";
